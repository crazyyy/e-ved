	...
	if(empty($exam->dont_store_data)) {
		...
		if(empty($taking_id)) {
			...
			$wpdb->query($wpdb->prepare("INSERT INTO ".WATU_TAKINGS." SET exam_id=%d, user_id=%d, ip=%s, date=CURDATE(), 
				points=%d, grade_id=%d, result=%s, snapshot='', start_time=%s, email=%s, percent_correct=%d", 
				$exam_id, $uid, $_SERVER['REMOTE_ADDR'], $achieved, $g_id, $grade, @$_POST['start_time'], $taker_email, $percent));
