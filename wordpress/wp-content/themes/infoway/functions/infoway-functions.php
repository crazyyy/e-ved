...
$delimiter = '&raquo;';
    $home = __( 'Home', 'infoway' ); // text for the 'Home' link
    ...
    if ( is_category() ) {
            ...
            echo $before . __( 'Archive by category', 'infoway') . ' "' . single_cat_title( '', false ) . '"' . $after;
    }
    elseif ( is_day() ) {
            ...
            echo $before . __( 'Search results for', 'infoway' ) . ' "' . get_search_query() . '"' . $after;
    } elseif ( is_tag() ) {
            echo $before . __( 'Posts tagged ', 'infoway' ) . '"' . single_tag_title( '', false ) . '"' . $after;
    } elseif ( is_author() ) {
            global $author;
            $userdata = get_userdata( $author );
            echo $before . __( 'Articles posted by ', 'infoway' ) . $userdata->display_name . $after;
    } elseif ( is_404() ) {
            echo $before . __( 'Error 404', 'infoway' ) . $after;
    }
...
// Retrieves tag list of current post, separated by commas.
    ...
    if ($tag_list) {
        $posted_in = __('This entry was posted in %1$s and tagged %2$s. Bookmark the ', 'infoway') . '<a href="%3$s" title="Permalink to %4$s" rel="bookmark">' . __('permalink', 'infoway') . '</a>';
    } elseif (is_object_in_taxonomy(get_post_type(), 'category')) {
        $posted_in = __('This entry was posted in %1$s. Bookmark the ', 'infoway') . '<a href="%3$s" title="Permalink to %4$s" rel="bookmark">' . __('permalink', 'infoway') . '</a>';
    } else {
        $posted_in = __('Bookmark the ', 'infoway') . '<a href="%3$s" title="Permalink to %4$s" rel="bookmark">' . __('permalink', 'infoway') . '</a>';
    }
 ...
 */
function infoway_wp_title($title, $sep) {
    global $page, $paged;
    if (is_feed())
        return $title;
    // Add the blog name
    ...
    if ($paged >= 2 || $page >= 2)
        $title .= " $sep " . sprintf(__('Page %s', 'infoway'), max($paged, $page));
    return $title;
}
add_filter('wp_title', 'infoway_wp_title', 10, 2);