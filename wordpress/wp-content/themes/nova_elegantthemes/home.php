		...
		<div class="container clearfix">
			<div id="services">
				<ul id="main-tabs">
					<?php 
						...
						$arr = array( 'post_type' => 'page',
									'orderby' => 'menu_order',
									'order' => 'ASC',
									'post__in' => get_option('nova_home_pages'),
					...
					<?php if (have_posts()) : while(have_posts()) : the_post(); ?>
						...
						<li><a href="#<?php #echo $hash; ?>"><strong><?php echo $tab_title; ?></strong><?php if ($tab_subtitle != '') { ?><span><?php echo $tab_subtitle; ?></span><?php } ?></a></li>
				...
				<?php for($i=0; $i < $home_pages_num; $i++) { ?>
					<div class="tab-slide clearfix" id="<?php echo $pagesContent[$i]['hash']; ?>">
						
						<?php if (!$pagesContent[$i]['portfolio']) { ?>
							<div class="content-area clearfix">
								<h3 class="title"><?php echo $pagesContent[$i]['title']; ?></h3>
								...
								<a href="<?php echo $pagesContent[$i]['link']; ?>" class="readmore"><span><?php _e('read more','Nova'); ?></span></a>
							</div> <!-- .content-area -->
						<?php } else { ?>				
							<div class="gallery-area clearfix">
								<?php $j = 0; ?>
								<?php query_posts("showposts=".$pagesContent[$i]['et_service_postsnum']."&cat=".$pagesContent[$i]['portfolio_categories']); ?>
									<?php if (have_posts()) : while(have_posts()) : the_post(); ?>
										<?php 
											$thumb = '';
											$width2 = 207;
										...
										<?php if ($thumb <> '') { ?>
											...
											<div class="et_pt_gallery_entry">
												<div class="et_pt_item_image">
													...
													<a class="fancybox zoom-icon" title="<?php the_title(); ?>" rel="gallery" href="<?php echo($thumbnail['fullpath']); ?>"><?php _e('Zoom in','Nova'); ?></a>
													<a class="more-icon" href="<?php the_permalink(); ?>"><?php _e('Read more','Nova'); ?></a>
					...
					<div id="entries-area-inner">
						<div id="entries-area-content" class="clearfix">
							<div id="content-area">							
								<?php 
									$args=array(
										   'showposts'=>get_option('nova_homepage_posts'),
										   'paged'=>$paged,
										   'category__not_in' => get_option('nova_exlcats_recent'),
									);
