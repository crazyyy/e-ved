...
/********* Page Templates v.1.0 ************/

add_action('wp_head','et_ptemplates_css');
function et_ptemplates_css(){
	echo('<link media="screen" type="text/css" href="'.get_bloginfo('template_directory').'/epanel/page_templates/page_templates.css" rel="stylesheet" />');
}

add_action('wp_print_scripts','et_ptemplates_footer_js');
function et_ptemplates_footer_js(){
	if ( !is_admin() ) {
		wp_register_script('et-ptemplates-frontend', get_bloginfo('template_directory').'/epanel/page_templates/js/et-ptemplates-frontend.js', array('jquery'), '1.0', true);
		wp_enqueue_script('et-ptemplates-frontend');
		wp_localize_script( 'et-ptemplates-frontend', 'ePtSettings', array(
			'path' => get_bloginfo('template_directory') . '/epanel/page_templates'
		));
	}
}
...
function et_ptemplate_save_details($post_id){
	...
	update_post_meta( $post->ID, "et_ptemplate_settings", maybe_serialize( $temp_array ) );
} ?>